...
router.get("/:id", async (req, res) => {
  try {
    const contactID = await Contact.findById(req.params.id); // find contact by id
    if (!contactID) {
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      const newContact = await Contact.create({ // if above is valid create a new contact
        user: req.user.id,
        name: req.body.name,
      ...
      const mailOptions = {
        ...
        text: 'We have received your message and appreciate your feedback regarding ' + req.body.subject
         + '.\n\nWe will get back to you shortly regarding your feedback / concerns.\n\nThank You! \n\nYour message is as follows.\n\n\n' 
         + req.body.message,
      };

...
router.post(
    ...
    async (req, res) => {
      ...
      try {
        ...
        // if (Date.parse(req.body.rentalDate) < today || Date.parse(req
        //     .body.returnDate) < today) {
        //   console.log(today);
...
router.patch(
    ...
    async (req, res) => {
      ...
      try {
        const rental = await Rental.findByIdAndUpdate(req
            .params.id, {$set: {returnDate: returnDate}});
        if (!rental) {
          return res.status(404).send("Rental not found");
        }
        await rental.save();
...
router.patch(
    ...
    [
      ...
      check("rating", "Rating needs to be a valid number between 0 and 5")
          .isInt({min: 0, max: 5}),
    ],
    async (req, res) => {
      const errors = validationResult(req);
      ...
      try {
        const rental = await Rental.findByIdAndUpdate(req.params
            .id, {$set: {comment:
                {title: title, text: text, rating: rating, date: date}}});
        if (!rental) {
          return res.status(404).send("Rental not found");
        }
        await rental.save();

        // The rating in this comment will go straight
        // to the overallRating array in the item Object
        const item = await Item.findByIdAndUpdate(rental
            .item.toString(), {$push: {overallRating: rating}});
        if (!item) {
          return res.status(404).send("Item not found");
        }
        await item.save();
...
router.patch(
    ...
    async (req, res) => {
      ...
      try {
        ...
        // This patch will check the Rental date and will
        // change isRented boolean accordingly
        const item = await Item.findById(rental.item.toString());
        if (!item) {
          return res.status(404).send("Item not found");
        }
        if (Date.parse(rental.rentalDate) <= today && Date
            .parse(rental.returnDate) >= today) {

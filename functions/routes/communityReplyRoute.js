...
const Community = require("../models/CommunityReply");
...
router.get("/", auth, async (req, res) => {
  try {
    const allCommunity = await Community.find({user: req.user.id});
...
router.get("/compost/:postid", async (req, res) => {
  try {
    const CommunityPostID = await Community.find({postid: req.params.postid});
    // find Community replies by post id
    if (!CommunityPostID) {
...
router.get("/all", async (req, res) => {
  try {
    const allUsersCommunity = await Community.find();
    // finds all Community in db
...
router.post(
    "/",
    auth,
    [check("reply", "Your reply is required").not().isEmpty()],
    [
    ...
    async (req, res) => {
      ...
      try {
        Community.count({postid: req.body.postid}, ( err, count) => {
          if (count == 0) {
            rCount = 2;
        ...
        // console.log( "Number of replies:", rCount,"post id:", req
        // .body.postid ); // works till here
        });
        ...
        console.log(err);
        rCount = 1;
        // issue where it doesntnnot reset the count when
        // new post is created and an reply is added

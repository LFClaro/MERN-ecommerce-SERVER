...
const Profile = require("../models/Profile"); // get access to our contact model
...
router.get("/", auth, async (req, res) => {
  try {
    const profile = await Profile
        .find({user: req.user.id}); // finds the Profile in db by the user id
    // res.send(profile);
    if (profile.length === 0) {
      res.send("null");
    } else {
...
router.get("/:id", async (req, res) => {
  try {
    const ProfileID = await Profile
        .findById(req.params.id); // find Profile by id
    if (!ProfileID) {
